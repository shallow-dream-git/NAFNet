...
class PairedStereoImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.lq_files =[lq[:4] for lq in os.listdir(self.lq_folder)]
        self.lq_files = sorted(set(self.lq_files), key=self.lq_files.index)

        self.gt_files =[gt[:4] for gt in os.listdir(self.gt_folder)]
        self.gt_files = sorted(set(self.gt_files), key=self.gt_files.index)
    ...
    def __getitem__(self, index):
        if self.file_client is None:
            self.file_client = FileClient(
                self.io_backend_opt.pop('type'), **self.io_backend_opt)
        gt_path_L = os.path.join(self.gt_folder, self.gt_files[index]+'_L.png')
        gt_path_R = os.path.join(self.gt_folder, self.gt_files[index]+'_R.png')
        ...
        lq_path_L = os.path.join(self.lq_folder, self.lq_files[index]+'_L.png')
        lq_path_R = os.path.join(self.lq_folder, self.lq_files[index]+'_R.png')
        ...
        if self.opt['use_channelshuffle'] and self.opt['phase'] == 'train':
            perm = np.array([0, 1, 2, 3, 4, 5]) if random.random() < 0.5 else np.random.permutation(6)
