...
class SCAM(nn.Module):
    ...
    def __init__(self, c):
        ...
        self.l_proj1 = nn.Conv2d(c, 3*c, kernel_size=1, stride=1, padding=0)
        self.l_dconv1 = nn.Conv2d(3*c, c, kernel_size=3, stride=1, padding=1, groups=c)
        self.r_proj1 = nn.Conv2d(c, 3*c, kernel_size=1, stride=1, padding=0)
        self.r_dconv1 = nn.Conv2d(3*c, c, kernel_size=3, stride=1, padding=1, groups=c)

        self.beta = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)

        self.l_proj2 = nn.Conv2d(c, 3*c, kernel_size=1, stride=1, padding=0)
        self.l_dconv2 = nn.Conv2d(3*c, c, kernel_size=3, stride=1, padding=1, groups=c)
        self.r_proj2 = nn.Conv2d(c, 3*c, kernel_size=1, stride=1, padding=0)
        self.r_dconv2 = nn.Conv2d(3*c, c, kernel_size=3, stride=1, padding=1, groups=c)
    def forward(self, x_l, x_r):
        Q_l = self.l_dconv1(self.l_proj1(self.norm_l(x_l))).permute(0, 2, 3, 1)  # B, H, W, c
        Q_r_T = self.r_dconv1(self.r_proj1(self.norm_r(x_r))).permute(0, 2, 1, 3) # B, H, c, W (transposed)

        V_l = self.l_dconv2(self.l_proj2(x_l)).permute(0, 2, 3, 1)  # B, H, W, c
        V_r = self.r_dconv2(self.r_proj2(x_r)).permute(0, 2, 3, 1)  # B, H, W, c
...
class NAFSSR(Local_Base, NAFNetSR, nn.Module):
